...
USER_NAME = os.environ['USER_NAME']  # 'animesh-1121'
QUERY_COUNT = {'user_getter': 0, 'follower_getter': 0, 'graph_repos_stars': 0, 'recursive_loc': 0, 'graph_commits': 0, 'loc_query': 0}


def daily_readme(since):
    """
    Returns the length of time since `since` (used for account 'uptime')
    ...
    return '{} {}, {} {}, {} {}{}'.format(
        ...
        ' \U0001F389' if (diff.months == 0 and diff.days == 0) else '')
...
def loc_query(owner_affiliation, force_cache=False, cursor=None, edges=[]):
...
def svg_overwrite(filename, uptime_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    root = tree.getroot()
    justify_format(root, 'uptime_data', uptime_data, 20)
